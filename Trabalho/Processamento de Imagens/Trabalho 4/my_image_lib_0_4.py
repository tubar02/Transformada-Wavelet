...
class Image:
	...
	def __sub__(self, imagem2):
		imagem_resultado = self._subtrai_matriz(self.pixels, imagem2.pixels) #faz a subtração

		lugar = self._local[:-4] + "Subtrai.pgm" #altera o local do resultado

		return self._cria_arquivo(lugar, imagem_resultado) #retorna uma nova imagem
	...
	def _subtrai_matriz(matriz1, matriz2): #subtrai duas matrizes
		linhas, colunas = len(matriz1), len(matriz1[0]) #recebe as dimensões da matriz

		assert linhas == len(matriz2) and colunas == len(matriz2[0]), "As dimensões das matrizes devem ser iguais"
	...
	def _cria_arquivo(self, outputpath, pixels):
		...
		with open(outputpath, "w") as arquivo:
			arquivo.write(self.magic + "\n") #escreve o número mágico
			arquivo.write(str(len(pixels[0])) + " " + str(len(pixels)) + "\n") #escreve as dimensões
...
class Fourier_Image(Image):
	def __init__(self, imagem): #passa a imagem que se deseja aplicar a transformada
		...
		if imagem._is_from_fourier: #caso tenha vindo de uma matriz complexa
			...
			self.matriz_fourier = Fourier_Image._fourier(imagem.pixels) #faz a matriz com os valores complexos
	...
	def representacao(self, do_shift = True): #faz uma imagem em escala de cinza de 1 a 255
	...
	def inverte(self, outputpath): #desfaz a transformada
		matriz = self._inv_fourier(self.matriz_fourier)

		return self._cria_arquivo(outputpath, matriz)
	
	#funções usadas dentro da classe
	@staticmethod
	def _fourier(matriz_pixels): #transformada de fourier
		copia = deepcopy(matriz_pixels)
		return fp.fft2(copia)
	
	@staticmethod
	def _inv_fourier(matriz_fourier):
		copia = deepcopy(matriz_fourier)
		return fp.ifft2(copia) #transformada inversa
