...
def main():
    ...
    for snr_in in snr_values:
        noisy_path = IMAGE_DIR / f"MRI_{snr_in}dB.pgm"
        noisy = ul.adiciona_ruido(original, mode="snr", param=snr_in,
                                  isImage=True, outputpath=str(noisy_path))
        save_image(noisy, OUTPUT_DIR / f"MRI_{snr_in}dB.png",
                   f"MRI {snr_in} dB")
        sigma = ul.snr(original, noisy, retorno="sigma", isImage=True)
        threshold = ul.visu_shrink(noisy, sigma, True)

        for wave in wavelets:
            wt = ul.aplica_DTWT_em_sinal(noisy, wave, level, True)
            save_wavelet_level(wt, level,
                               OUTPUT_DIR / f"WT_{wave}_L{level}_{snr_in}dB_before.png",
                               f"WT {wave} L{level} {snr_in}dB - Antes")
            for method_name, method in methods.items():
                wt_filtered = method(deepcopy(wt), threshold, True)
                save_wavelet_level(
                    wt_filtered, level,
                    OUTPUT_DIR / f"WT_{wave}_L{level}_{snr_in}dB_{method_name}.png",
                    f"WT {wave} L{level} {snr_in}dB - {method_name}")
                rec_path = IMAGE_DIR / f"MRI_{snr_in}dB_{wave}_L{level}_{method_name}.pgm"
                recon = ul.aplica_IDTWT_em_sinal(wt_filtered, isImage=True,
                                                  outputpath=str(rec_path))
                save_image(
                    recon,
                    OUTPUT_DIR / f"MRI_{snr_in}dB_{wave}_L{level}_{method_name}.png",
                    f"Rec {wave} L{level} {method_name} {snr_in}dB")
